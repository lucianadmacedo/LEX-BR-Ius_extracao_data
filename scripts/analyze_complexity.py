...
class ComplexityAnalyzer:
    ...
    def analyze_complex_verb_forms(self, sentence):
        """Count subjunctive, future subjunctive, conditional forms"""
        complexity = 0
    ...
    def analyze_subordination_depth(self, sentence):
        ...
        # Build dependency tree and find max depth
        depths = {}
    ...
    def calculate_normalized_metrics(self):
        """Calculate per-sentence and per-token normalized metrics"""
        normalized = {}
    ...
    def generate_report(self, output_file='complexity_analysis_report.txt'):
        ...
        with open(output_file, 'w', encoding='utf-8') as f:
            ...
            f.write("Brazilian Legal Texts (1943-2009)\n")
            f.write("=" * 80 + "\n\n")

            # Overall statistics
            f.write("OVERALL STATISTICS BY YEAR\n")
            f.write("-" * 80 + "\n")
            f.write(f"{'Year':<8} {'Files':<8} {'Sentences':<12} {'Tokens':<12} {'Avg Sent Len':<15}\n")
            f.write("-" * 80 + "\n")

            for year in sorted(normalized.keys()):
                data = normalized[year]
                f.write(f"{year:<8} {data['files']:<8} {data['sentences']:<12} "
                       f"{data['tokens']:<12} {data['avg_sentence_length']:<15.2f}\n")
            ...
            # Trends summary
            f.write("TEMPORAL TRENDS SUMMARY\n")
            f.write("-" * 80 + "\n")
            ...
            if len(years) >= 2:
                ...
                f.write(f"\nComparison: {first_year} vs {last_year}\n\n")

                for feature_key, feature_name in features:
                    first_val = normalized[first_year]['per_sentence'][feature_key]
                    last_val = normalized[last_year]['per_sentence'][feature_key]

                    if first_val > 0:
                        change_pct = ((last_val - first_val) / first_val) * 100
                        direction = "↑" if change_pct > 0 else "↓"
                        f.write(f"{feature_name}:\n")
                        f.write(f"  {first_year}: {first_val:.3f} → {last_year}: {last_val:.3f} "
